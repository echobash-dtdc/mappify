...
@section('content')
    <div class="py-12 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="py-12 bg-white">
                <a href="{{ route('maps.list') }}" class="bg-blue-500 text-white px-4 py-4 rounded hover:bg-blue-600">See on Map</a>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
                @forelse ($maps as $map)
                    <div class="flex flex-col items-center text-center">
                        ...
                        <p class="italic text-sm text-gray-500">{{ $map->category ?? 'Map' }}</p>
                        <p class="mt-2 text-gray-600">{{ $map->description }}</p>
                    </div>
                @empty
